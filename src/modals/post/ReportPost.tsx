...
const ReportPost = ({ isOpen, onClose,postId }: { isOpen: boolean; onClose: () => void;postId:string}) => {
    ...
    const handleChange = (event: any) => {
        ...
        setError(''); // Clear error message when a reason is selected
    };
                        ...
                        {reportReasons.map((reason, index) => (
                            <div key={index} className="flex items-center space-x-3 py-3">
                                <input
                                    ...
                                    className={`w-4 h-4 border-gray-300 dark:bg-gray-600 dark:border-gray-500 ${
                                        selectedReason === reason ? 'text-blue-600' : 'text-transparent'
                                    }`}
                                />
                                <label
